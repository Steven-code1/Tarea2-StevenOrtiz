...
public class BookRest {
    ...
    @Fallback(fallbackMethod = "findAllCompletoFallback")
    @Path("/all")
...
//                .build( AuthorRestProxy.class );
        ...
        List<BookDto> ret = books.stream()
                .map(s -> {
                    System.out.println("*********buscando consultando Fallback" + s.getId() );
    ...
    @GET
    @Path("/allFallback")
    public List<BookDto> findAllCompletoFallback() throws Exception {
        var books = bookService.findAll();
